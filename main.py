...
def setup_scheduler():
    ...
    tasks = [
        ...
        (Data_Collection_Script.run, CronTrigger(hour=12, minute=0, second=0)),
        # Execute every hour HH:10:00
        (jobbankProvince_fetcher.main, CronTrigger(minute=10, second=0)),
        # Execute every hour HH:20:00
        (Data_Loading_Script.main, CronTrigger(minute=20, second=0)),
        # Execute every hour HH:30:00
        (Data_Cleaning_Script.main, CronTrigger(minute=30, second=0)),
        # Execute every hour HH:40:00
        (Load_Cleaned_Data.main, CronTrigger(minute=40, second=0)),
    ]
    for task in tasks:
        job = task[0]
        trigger = task[1]
